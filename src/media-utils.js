  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      ffmpeg()
        ...
        .inputFormat('lavfi')
        .outputOptions(
          staticWebpStickerOptions(
            `drawtext=textfile='${escapedTextPath}':fontcolor=white:fontsize=${fontSize}:borderw=4:bordercolor=black:line_spacing=10:x=(w-text_w)/2:y=(h-text_h)/2,format=rgba`
          )
        )
        .format('webp')
        .save(outputPath)
...
export const videoToSticker = async (buffer) =>
  bufferToStickerWithFfmpeg({
    ...
    configure: (command) => {
      command
        .inputOptions(['-t 6'])
        .outputOptions(animatedWebpStickerOptions(`fps=12,${stickerSizeFilter}`))
        .format('webp');
    }
  });
